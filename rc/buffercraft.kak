...
buffercraft-alternate %{
  execute-keys -with-hooks '<esc>: edit %sh{printf %s "$kak_opt_buffercraft_alternate" | kak-buffercraft "$kak_opt_buffercraft_pattern" "$kak_buffile"}<ret>'
}

define-command -docstring "write the buffer template to the current filename" \
buffercraft-template %{
  execute-keys -with-hooks '<esc>: echo -to-file %val{buffile} %sh{printf %s "$kak_opt_buffercraft_template" | kak-buffercraft "$kak_opt_buffercraft_pattern" "$kak_buffile"}<ret>'
}
